...
import com.example.dell.lotteryplan.home.adapter.HappyAdpater;
...
public class HappytimeFagment extends Fragment {

    private View                    rootView;
    private ListView                happyListView;
    private HappyAdpater            happyAdpater;
    private List<LotteryMessage>    happyList  = new ArrayList<>();
    ...
    private long[]                  min             = new long[7];
    private long[]                  secd            = new long[7];
    private String[]                iss             = new String[7];
    private String[]                tempIss         = new String[7];
    //上期开奖数据
    private String                  server_time;
    private String                  issue           = null;
    private String                  num             = null;
    private String                  expect_time     = null;
    private String[]                lastTime        = new String[7];
    private String[]                templastTime    = new String[7];
    //本期开奖数据
    private String                  issueNow        = null;
    private String                  expect_timeNow  = null;
    private boolean[]               isNextOpen      = new boolean[7];
    private boolean[]               isFistTime      = new boolean[7];
    private boolean[]               isLastTime      = new boolean[7];
    private boolean[]               isChanger       = new boolean[7];
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (rootView == null) {
            rootView = inflater.inflate(R.layout.happytime_fragment, container, false);
            initView(rootView);
        }
    ...
    public void setData(){
        ...
        for(int i=0;i<homeTitles.size();i++){
            ...
            happyList.add(lotteryMessage);
        }
    }

    //设置列表
    public void setListView(View view){
        happyListView = (ListView)view.findViewById(R.id.happyListView);
        happyListView.getListView().setDividerHeight(0);
        happyAdpater  =  new HappyAdpater(this.getContext(),happyList);
        happyListView.getListView().setAdapter(happyAdpater);
        happyAdpater.notifyDataSetChanged();
        happyListView.getListView().setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {

            }
        });
        happyListView.setLoadDataListener(new PullToRefreshLayout.LoadDataListener() {
            @Override
            //下拉刷新调用
        ...
        happyListView.getListView().setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                switch (scrollState) {
                    case AbsListView.OnScrollListener.SCROLL_STATE_FLING:
                        happyAdpater.setmBusy(true);
                        break;
                    case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:
                        happyAdpater.setmBusy(false);
                        break;
    ...
    public void requestLotteryType(){
        ...
        httpTask.execute(Constants.LOTTERY_TYPE+"4");
        ...
        Httptask2.setTaskHandler(new HttpTask.HttpTaskHandler() {
            public void taskSuccessful(String json) {
                try {
                    ...
                    happyListView.onLoadComplete(true, false);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public void setNowMessage(String home_date, String expect_time, int pos) {
        try {
            ...
            min[pos]        = minutes;
            secd[pos]       = seconds;
            happyList.get(pos).setNow_time(nowTime);
            happyList.get(pos).setNext_time(home_date);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void setMessage(String[] num, String issue, int pos) {
        ...
        for(int i=0;i<num.length;i++){
            if(num[i].equals("19") || num[i].equals("20")){
                happyList.get(pos).setSpecil(true,i);
            }else{
                happyList.get(pos).setSpecil(false,i);
            }
        }
        setBall(num,pos);
        if(!isNextOpen[pos]){
            happyList.get(pos).setLottery_issue("第 " + lastTime[pos] + " 期开奖中");
        }else{
            happyList.get(pos).setOpen(false);
            happyList.get(pos).setLottery_issue("第 " + lastTime[pos] + " 期已开");
        }
    }

    //设置球码
    public void setBall(String[] num,int pos){
        happyList.get(pos).clearball();
        for(int i=0;i<num.length;i++){
            happyList.get(pos).setBall(num[i]);
        }
    }
    ...
    public void reBackTime(int pos) {
        if(hours[pos] <= 0){
            happyList.get(pos).setHour("00");
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    happyList.get(pos).setMinute(min[pos]+"");
                    happyList.get(pos).setSecond(secd[pos]+"");
                } else {
                    happyList.get(pos).setMinute(min[pos]+"");
                    happyList.get(pos).setSecond("0" +secd[pos]+"");
                }
            } else if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] < 10 && min[pos] >= 0) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    happyList.get(pos).setMinute("0" +min[pos]+"");
                    happyList.get(pos).setSecond(secd[pos]+"");
                } else {
                    happyList.get(pos).setMinute("0" +min[pos]+"");
                    happyList.get(pos).setSecond("0" +secd[pos]+"");
                ...
                happyList.get(pos).setMinute("00");
                happyList.get(pos).setSecond("00");
                happyList.get(pos).setOpen(true);
                requestLotteryData(pos);
            }
        }else if(hours[pos]>0 && hours[pos] <10){
            happyList.get(pos).setHour("0"+hours[pos]);
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    happyList.get(pos).setMinute(min[pos]+"");
            ...
            happyList.get(pos).setHour(hours[pos]+"");
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    happyList.get(pos).setMinute(min[pos]+"");
    ...
    private static class DataHandler extends Handler {
        WeakReference<HappytimeFagment> reference;

        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            HappytimeFagment lottery = reference.get();
            if (lottery != null) {
                lottery.requestData();
                lottery.happyAdpater.notifyDataSetChanged();
                lottery.dataHandler.postDelayed(lottery.dataRunnable, 15000);
            }
        }

        DataHandler(HappytimeFagment lottery) {
    ...
    private static class TimeHandler extends Handler {
        WeakReference<HappytimeFagment> reference;
        public TimeHandler(HappytimeFagment lottery) {
            reference = new WeakReference<>(lottery);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            HappytimeFagment lottery = reference.get();
            if (lottery != null) {
                if(lottery.happyList.size()!=0){
                    for (int i = 0; i < lottery.happyList.size(); i++) {
