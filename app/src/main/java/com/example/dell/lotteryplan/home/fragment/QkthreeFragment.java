...
import com.example.dell.lotteryplan.home.adapter.QKThreeAdapter;
...
public class QkthreeFragment extends Fragment {

    private View                    rootView;
    private ListView                qkthreeListView;
    private QKThreeAdapter          qkThreeAdapter;
    private List<LotteryMessage>    qkthreeList = new ArrayList<>();
    private HttpTask                httpTask1,httpTask2,Httptask1,Httptask2;
    //开奖信息
    //请求时间数据
    ...
    private long[]                  hours           = new long[13];
    private long[]                  min             = new long[13];
    private long[]                  secd            = new long[13];
    private String[]                iss             = new String[13];
    private String[]                tempIss         = new String[13];
    //上期开奖数据
    private String                  server_time;
    private String                  issue           = null;
    private String                  num             = null;
    private String                  expect_time     = null;
    private String[]                lastTime        = new String[13];
    private String[]                templastTime    = new String[13];
    //本期开奖数据
    private String                  issueNow        = null;
    private String                  expect_timeNow  = null;
    private boolean[]               isNextOpen      = new boolean[13];
    private boolean[]               isFistTime      = new boolean[13];
    private boolean[]               isLastTime      = new boolean[13];
    private boolean[]               isChanger       = new boolean[13];
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (rootView == null) {
            rootView = inflater.inflate(R.layout.qkthree_fragment, container, false);
            initView(rootView);
        }
    ...
    public void setData(){
        ...
        for(int i=0;i<homeTitles.size();i++){
            ...
            qkthreeList.add(lotteryMessage);
        }
    }

    //设置列表
    public void setListView(View view){
        qkthreeListView = (ListView)view.findViewById(R.id.qkthreeListView);
        qkthreeListView.getListView().setDividerHeight(0);
        qkThreeAdapter  =  new QKThreeAdapter(this.getContext(),qkthreeList);
        qkthreeListView.getListView().setAdapter(qkThreeAdapter);
        qkThreeAdapter.notifyDataSetChanged();
        qkthreeListView.getListView().setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {

            }
        });
        qkthreeListView.setLoadDataListener(new PullToRefreshLayout.LoadDataListener() {
            @Override
            //下拉刷新调用
        ...
        qkthreeListView.getListView().setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                switch (scrollState) {
                    case AbsListView.OnScrollListener.SCROLL_STATE_FLING:
                        qkThreeAdapter.setmBusy(true);
                        break;
                    case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:
                        qkThreeAdapter.setmBusy(false);
                        break;
        ...
        Httptask2.setTaskHandler(new HttpTask.HttpTaskHandler() {
            public void taskSuccessful(String json) {
                try {
                    ...
                    qkthreeListView.onLoadComplete(true, false);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public void requestLotteryType(){
        ...
        httpTask.execute(Constants.LOTTERY_TYPE+"2");
    ...
    public void setNowMessage(String home_date, String expect_time, int pos) {
        try {
            ...
            min[pos]        = minutes;
            secd[pos]       = seconds;
            qkthreeList.get(pos).setNow_time(nowTime);
            qkthreeList.get(pos).setNext_time(home_date);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void setMessage(String[] num, String issue, int pos) {
        ...
        setBall(num,pos);
        if(!isNextOpen[pos]){
            qkthreeList.get(pos).setLottery_issue("第 " + lastTime[pos] + " 期开奖中");
        }else{
            qkthreeList.get(pos).setOpen(false);
            qkthreeList.get(pos).setLottery_issue("第 " + lastTime[pos] + " 期已开");
        }
    }

    //设置球码
    public void setBall(String[] num,int pos){
        qkthreeList.get(pos).clearball();
        for(int i=0;i<num.length;i++){
            qkthreeList.get(pos).setBall(num[i]);
        }
    }
    ...
    public void reBackTime(int pos) {
        if(hours[pos] == 0){
            qkthreeList.get(pos).setHour("00");
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    qkthreeList.get(pos).setMinute(min[pos]+"");
                    qkthreeList.get(pos).setSecond(secd[pos]+"");
                } else {
                    qkthreeList.get(pos).setMinute(min[pos]+"");
                    qkthreeList.get(pos).setSecond("0" +secd[pos]+"");
                }
            } else if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] < 10 && min[pos] >= 0) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    qkthreeList.get(pos).setMinute("0" +min[pos]+"");
                    qkthreeList.get(pos).setSecond(secd[pos]+"");
                } else {
                    qkthreeList.get(pos).setMinute("0" +min[pos]+"");
                    qkthreeList.get(pos).setSecond("0" +secd[pos]+"");
                ...
                qkthreeList.get(pos).setMinute("00");
                qkthreeList.get(pos).setSecond("00");
                qkthreeList.get(pos).setOpen(true);
                requestLotteryData(pos);
            }
        }else if(hours[pos]>0 && hours[pos] <10){
            qkthreeList.get(pos).setHour("0"+hours[pos]);
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    qkthreeList.get(pos).setMinute(min[pos]+"");
            ...
            qkthreeList.get(pos).setHour(hours[pos]+"");
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    qkthreeList.get(pos).setMinute(min[pos]+"");
    ...
    private static class DataHandler extends Handler {
        WeakReference<QkthreeFragment> reference;

        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            QkthreeFragment lottery = reference.get();
            if (lottery != null) {
                lottery.requestData();
                lottery.qkThreeAdapter.notifyDataSetChanged();
                lottery.dataHandler.postDelayed(lottery.dataRunnable, 15000);
            }
        }

        DataHandler(QkthreeFragment lottery) {
    ...
    private static class TimeHandler extends Handler {
        WeakReference<QkthreeFragment> reference;
        public TimeHandler(QkthreeFragment lottery) {
            reference = new WeakReference<>(lottery);
        }
