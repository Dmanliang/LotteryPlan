...
public class LotteryFragment extends Fragment {
    ...
    private long[]                  hours           = new long[4];
    private long[]                  min             = new long[4];
    private long[]                  secd            = new long[4];
    private String[]                iss             = new String[4];
    private String[]                tempIss         = new String[4];
    //上期开奖数据
    private String                  server_time;
    private String                  issue           = null;
    private String                  num             = null;
    private String                  expect_time     = null;
    private String[]                lastTime        = new String[4];
    private String[]                templastTime    = new String[4];
    //本期开奖数据
    private String                  issueNow        = null;
    private String                  expect_timeNow  = null;
    private boolean[]               isNextOpen      = new boolean[4];
    private boolean[]               isFistTime      = new boolean[4];
    private boolean[]               isLastTime      = new boolean[4];
    private boolean[]               isChanger       = new boolean[4];
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (rootView == null) {
            rootView = inflater.inflate(R.layout.lottery_fragment, container, false);
            initView(rootView);
        }
    ...
    public void setListView(View view){
        lotteryListView = (ListView)view.findViewById(R.id.lotteryListView1);
        lotteryListView.getListView().setDividerHeight(0);
        lotteryAdpater  =  new LotteryAdpater(this.getContext(),lotteryList);
        lotteryListView.getListView().setAdapter(lotteryAdpater);
        lotteryAdpater.notifyDataSetChanged();
        lotteryListView.getListView().setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {

            }
        });
        lotteryListView.setLoadDataListener(new PullToRefreshLayout.LoadDataListener() {
            @Override
            //下拉刷新调用
        ...
        lotteryListView.getListView().setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
    ...
    public void requestLotteryType(){
        ...
        httpTask.execute(Constants.LOTTERY_TYPE+"1");
        ...
        Httptask2.setTaskHandler(new HttpTask.HttpTaskHandler() {
            public void taskSuccessful(String json) {
                try {
                    ...
                    lotteryListView.onLoadComplete(true, false);
                    if(!isRun){
                        isRun = true;
                        Message msg = new Message();
                        msg.what = 2;
    ...
    WeakReference<LotteryFragment> reference;

    @Override
    public void handleMessage(Message msg) {
        super.handleMessage(msg);
        LotteryFragment lottery = reference.get();
        if (lottery != null) {
            lottery.requestData();
    ...
    DataHandler(LotteryFragment lottery) {
    ...
    private static class TimeHandler extends Handler {
        WeakReference<LotteryFragment> reference;
        public TimeHandler(LotteryFragment lottery) {
            reference = new WeakReference<>(lottery);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            if (lottery != null) {
                if(lottery.lotteryList.size()!=0){
                    for (int i = 0; i < lottery.lotteryList.size(); i++) {
