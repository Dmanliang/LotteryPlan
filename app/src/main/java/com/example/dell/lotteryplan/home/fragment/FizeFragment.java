...
public class FizeFragment extends Fragment {

    private View                    rootView;
    private ListView                fizeListView;
    private LotteryAdpater          lotteryAdpater;
    private List<LotteryMessage>    lotteryList  = new ArrayList<>();
    ...
    private long[]                  hours           = new long[22];
    private long[]                  min             = new long[22];
    private long[]                  secd            = new long[22];
    private String[]                iss             = new String[22];
    private String[]                tempIss         = new String[22];
    //上期开奖数据
    private String                  server_time;
    private String                  issue           = null;
    private String                  num             = null;
    private String                  expect_time     = null;
    private String[]                lastTime        = new String[22];
    private String[]                templastTime    = new String[22];
    //本期开奖数据
    private String                  issueNow        = null;
    private String                  expect_timeNow  = null;
    private boolean[]               isNextOpen      = new boolean[22];
    private boolean[]               isFistTime      = new boolean[22];
    private boolean[]               isLastTime      = new boolean[22];
    private boolean[]               isChanger       = new boolean[22];
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (rootView == null) {
            rootView = inflater.inflate(R.layout.fize_fragment, container, false);
            initView(rootView);
        }
    ...
    public void setData(){
        ...
        for(int i=0;i<homeTitles.size();i++){
            ...
            lotteryList.add(lotteryMessage);
        }
    }

    //设置列表
    public void setListView(View view){
        fizeListView = (ListView)view.findViewById(R.id.fizeListView);
        fizeListView.getListView().setDividerHeight(0);
        lotteryAdpater  =  new LotteryAdpater(this.getContext(),lotteryList);
        fizeListView.getListView().setAdapter(lotteryAdpater);
        lotteryAdpater.notifyDataSetChanged();
        fizeListView.getListView().setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {

            }
        });
        fizeListView.setLoadDataListener(new PullToRefreshLayout.LoadDataListener() {
            @Override
            //下拉刷新调用
        ...
        fizeListView.getListView().setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                switch (scrollState) {
                    case AbsListView.OnScrollListener.SCROLL_STATE_FLING:
                        lotteryAdpater.setmBusy(true);
                        break;
                    case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:
                        lotteryAdpater.setmBusy(false);
                        break;
        ...
        Httptask2.setTaskHandler(new HttpTask.HttpTaskHandler() {
            public void taskSuccessful(String json) {
                try {
                    ...
                    fizeListView.onLoadComplete(true, false);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    public void requestLotteryType(){
        ...
        httpTask.execute(Constants.LOTTERY_TYPE+"3");
    ...
    public void setNowMessage(String home_date, String expect_time, int pos) {
        try {
            ...
            min[pos]        = minutes;
            secd[pos]       = seconds;
            lotteryList.get(pos).setNow_time(nowTime);
            lotteryList.get(pos).setNext_time(home_date);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void setMessage(String[] num, String issue, int pos) {
        ...
        setBall(num,pos);
        if(!isNextOpen[pos]){
            lotteryList.get(pos).setLottery_issue("第 " + lastTime[pos] + " 期开奖中");
        }else{
            lotteryList.get(pos).setOpen(false);
            lotteryList.get(pos).setLottery_issue("第 " + lastTime[pos] + " 期已开");
        }
    }

    //设置球码
    public void setBall(String[] num,int pos){
        lotteryList.get(pos).clearball();
        for(int i=0;i<num.length;i++){
            lotteryList.get(pos).setBall(num[i]);
        }
    }
    ...
    public void reBackTime(int pos) {
        if(hours[pos] <= 0){
            lotteryList.get(pos).setHour("00");
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    lotteryList.get(pos).setMinute(min[pos]+"");
                    lotteryList.get(pos).setSecond(secd[pos]+"");
                } else {
                    lotteryList.get(pos).setMinute(min[pos]+"");
                    lotteryList.get(pos).setSecond("0" +secd[pos]+"");
                }
            } else if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] < 10 && min[pos] >= 0) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    lotteryList.get(pos).setMinute("0" +min[pos]+"");
                    lotteryList.get(pos).setSecond(secd[pos]+"");
                } else {
                    lotteryList.get(pos).setMinute("0" +min[pos]+"");
                    lotteryList.get(pos).setSecond("0" +secd[pos]+"");
                ...
                lotteryList.get(pos).setMinute("00");
                lotteryList.get(pos).setSecond("00");
                lotteryList.get(pos).setOpen(true);
                requestLotteryData(pos);
            }
        }else if(hours[pos]>0 && hours[pos] <10){
            lotteryList.get(pos).setHour("0"+hours[pos]);
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    lotteryList.get(pos).setMinute(min[pos]+"");
            ...
            lotteryList.get(pos).setHour(hours[pos]+"");
            if (secd[pos] <= 60 && secd[pos] > 0 && min[pos] >= 10) {
                secd[pos]--;
                if (secd[pos] >= 10) {
                    lotteryList.get(pos).setMinute(min[pos]+"");
    ...
    private static class DataHandler extends Handler {
        WeakReference<FizeFragment> reference;
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            FizeFragment lottery = reference.get();
            if (lottery != null) {
                lottery.requestData();
                lottery.lotteryAdpater.notifyDataSetChanged();
                lottery.dataHandler.postDelayed(lottery.dataRunnable, 15000);
            }
        }

        DataHandler(FizeFragment lottery) {
    ...
    private static class TimeHandler extends Handler {
        WeakReference<FizeFragment> reference;
        public TimeHandler(FizeFragment lottery) {
            reference = new WeakReference<>(lottery);
        }
